...
class Environment_Gaussian:
    ...
    def response(self, arm):
        ...
        reward = self.random_generator.normal(loc=0.0, scale=1) + self.rlist[arm - 1]
        return reward
...
class Environment_Bernoulli:
    ...
    def response(self, arm):
        # vectorize binomial sample function to accelerate
        reward = self.random_generator.binomial(n=1, p=self.rlist[arm - 1])
        return reward


class Environment_Constant:
    # the demand follows Bernoulli Distribution
    def __init__(self, rlist: np.array, K: int, random_seed=12345):
        """The environment that return gaussian reward
